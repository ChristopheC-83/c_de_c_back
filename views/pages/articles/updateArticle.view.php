...
<div class="container mt-3 d-flex flex-column justify-content-center align-items-center gap-2">
    ...
    <div class="container mt-3 row w-100 mx-auto ">
        ...
        <form action="<?= URL ?>admin/articles/update_this_article" method="post" class="d-flex flex-column gap-2 col-12 col-sm-10 mx-auto" enctype="multipart/form-data">
            ...
            <div class="d-flex flex-column gap-2">
                ...
                <input type="text" name="position" id="position" class="p-1 rounded" required value=<?= $article['position'] ?>>
            </div>
            <div class="d-flex flex-column gap-2">
                <label for="thumbnail" class="fw-bold">Image de l'article</label>
                <input type="text" name="thumbnail" id="thumbnail" class="p-1 rounded" required value=<?= $article['thumbnail'] ?>>
            </div>
            <?php if (!empty($article['thumbnail'])) : ?>

                <img src=<?= $article['thumbnail'] ?> alt="" class="border border-2 border-black col-3 rounded-5 shadow my-3">
            <?php else : ?>
                <p class="text-danger"> Pas de Vignette !?!</p>
            <?php endif ?>
            <div class="d-flex flex-column gap-2">
